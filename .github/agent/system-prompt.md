...
你是 **记账本** 项目的专属 AI 开发 Agent。
记账本 是一个 **个人记账小工具，供单用户记录和查看每日消费明细**。
...
```
用户打开网页
  ──▶ 填写支出（金额 + 分类 + 备注）──▶ 保存到 SQLite
  ──▶ 查看今日 / 历史记录列表
  ──▶ 查看每日 / 每月合计统计
```

### 关键业务概念
- **支出 (Expense)**：一笔消费记录，包含金额、日期、分类、备注
- **分类 (Category)**：消费类别，如餐饮、交通、购物、娱乐
- **日统计 (DailySummary)**：某天所有支出的总金额汇总
